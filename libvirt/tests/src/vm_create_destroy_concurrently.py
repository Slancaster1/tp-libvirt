...
def create_scripts(vmxml, num_scripts, timeout):
    ...
    for i in range(num_scripts):
        ...
        nvram_path = tmp_dir + "/nvram_" + str(i) + ".qcow2"

        # SELinux does not allow multiple VM operations the same qcow2 files
        vmxml.remove_all_disk()

        try:
            shutil.copy(vmxml.os.nvram, nvram_path)
            attrs = {"nvram": nvram_path}
            vmxml.set_os_attrs(**attrs)
            logging.info("NVRAM Path: {}".format(vmxml.os.nvram))
        except xcepts.LibvirtXMLNotFoundError:
            pass  # VM defined without NVRAM
...
def get_pids_for(names):
    ...
    relevent_procs = [x for x in ps_output for n in names if n in x]
    relevent_procs = [x.split() for x in relevent_procs]
    relevent_pids = [int(x[1]) for x in relevent_procs]
    relevent_pids.sort()
    logging.info("Processes and Pids matching {}: {}".format(names, relevent_procs))

    return relevent_pids


def get_libvirt_pids(test, daemon):
    """
    Given a Libvirtd daemon object, retrieve the
    pids relevent to the libvirt services
